 ...
 */
export const equalsEpsilon = function (left: number, right: number, relativeEpsilon?: number, absoluteEpsilon?: number): boolean {
    // >>includeStart('debug', pragmas.debug);
    if (!defined(left)) {
    ...
    return absDiff <= (absoluteEpsilon as number) || absDiff <= (absoluteEpsilon as number) * Math.max(Math.abs(left), Math.abs(right));
};
...
const CesiumMath = {
     ...
     * @returns {Number} The sign of value.
     */
    sign: defaultValue(Math.sign, function sign(value: number) {
