...
class TweenCollection {
     ...
     */
    add(options: { startObject?: unknown; stopObject?: { [name: string]: any }; duration?: number; delay?: number; easingFunction?: number; update?: any; complete?: any; cancel?: any; _repeat?: number }): Tween {
        ...
        const easingFunction = defaultValue(options.easingFunction, EasingFunction.LINEAR_NONE as unknown) as any;
    ...
    update(time?: number): void {
        ...
        let i = 0;
        time = defined(time) ? (time as number) / TimeConstants.SECONDS_PER_MILLISECOND : getTimestamp();
