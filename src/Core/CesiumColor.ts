...
class CesiumColor {
    ...
    static fromCssColorString(color: string, result?: CesiumColor | undefined): CesiumColor | undefined {
        ...
        const namedColor = (CesiumColor as any)[color.toUpperCase()];
        ...
        if (matches !== null) {
            (result as CesiumColor).red = parseFloat(matches[1]) / (matches[1].substr(-1) === '%' ? 100.0 : 255.0);
            (result as CesiumColor).green = parseFloat(matches[2]) / (matches[2].substr(-1) === '%' ? 100.0 : 255.0);
            (result as CesiumColor).blue = parseFloat(matches[3]) / (matches[3].substr(-1) === '%' ? 100.0 : 255.0);
        ...
        if (matches !== null) {
            return CesiumColor.fromHsl(parseFloat(matches[1]) / 360.0, parseFloat(matches[2]) / 100.0, parseFloat(matches[3]) / 100.0, parseFloat(defaultValue(matches[4], '1.0')), result);
        }
