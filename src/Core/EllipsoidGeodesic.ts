...
class EllipsoidGeodesic {
    ...
    _distance?: number;
    _uSquared: any;
    constructor(start: any, end: any, ellipsoid?: any) {
    ...
    get surfaceDistance(): number {
        return this._distance as number;
    }
    ...
    get startHeading(): number {
        return this._startHeading as number;
    }

    get endHeading(): number {
        return this._endHeading as number;
    }
    ...
    interpolateUsingFraction(fraction: number, result?: Cartographic): Cartographic {
        return this.interpolateUsingSurfaceDistance((this._distance as number) * fraction, result);
    }
    ...
    interpolateUsingSurfaceDistance(distance: number, result?: Cartographic): Cartographic {
        ...
        let sigma =
            ...
            s * (1.0 - u2Over4 + (7.0 * u4Over16) / 4.0 - (15.0 * u6Over64) / 4.0 + (579.0 * u8Over256) / 64.0 - (u4Over16 - (15.0 * u6Over64) / 4.0 + (187.0 * u8Over256) / 16.0) * cosine2S - ((5.0 * u6Over64) / 4.0 - (115.0 * u8Over256) / 16.0) * cosine4S - (29.0 * u8Over256 * cosine6S) / 16.0) +
        ...
        if (defined(result)) {
            (result as Cartographic).longitude = this._start.longitude + l;
