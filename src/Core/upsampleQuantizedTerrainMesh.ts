...
const clipScratch: any[] = [];
const clipScratch2: any[] = [];
const verticesScratch: any[] = [];
const cartographicScratch = new Cartographic();
let cartesian3Scratch = new Cartesian3();
const uScratch: any[] = [];
const vScratch: any[] = [];
const heightScratch: any[] = [];
const indicesScratch: any[] = [];
const normalsScratch: any[] = [];
...
function upsampleQuantizedTerrainMesh(parameters: any, transferableObjects?: any) {
    ...
    const parentNormalBuffer: any = hasVertexNormals ? new Array(quantizedVertexCount * 2) : undefined;
    ...
    for (i = 0; i < parentIndices.length; i += 3) {
        ...
        const clipped = Intersections2D.clipTriangleAtAxisAlignedThreshold(halfMaxShort, isEastChild, u0, u1, u2, clipScratch);
    ...
    const rectangle = Rectangle.clone(parameters.childRectangle) as Rectangle;
    ...
    const boundingSphere = BoundingSphere.fromVertices(cartesianVertices, Cartesian3.ZERO, 3, boundingSphereScratch);
    const orientedBoundingBox = OrientedBoundingBox.fromRectangle(rectangle, minimumHeight, maximumHeight, ellipsoid, orientedBoundingBoxScratch);

    const occluder = new EllipsoidalOccluder(ellipsoid);
    const horizonOcclusionPoint = occluder.computeHorizonCullingPointFromVerticesPossiblyUnderEllipsoid(boundingSphere.center, cartesianVertices, 3, boundingSphere.center, minimumHeight, horizonOcclusionPointScratch);
...
class Vertex {
    ...
    initializeIndexed(uBuffer: any, vBuffer: any, heightBuffer: any, normalBuffer: any, index: any) {
    ...
    initializeFromClipResult(clipResult: any, index: any, vertices: any) {
...
const polygonVertices: any = [];
polygonVertices.push(new Vertex());
polygonVertices.push(new Vertex());
polygonVertices.push(new Vertex());
polygonVertices.push(new Vertex());

function addClippedPolygon(uBuffer: any, vBuffer: any, heightBuffer: any, normalBuffer: any, indices: any, vertexMap: any, clipped: any, triangleVertices: any, hasVertexNormals: any) {
    if (clipped.length === 0) {
        return;
    }
...
function lerpOctEncodedNormal(vertex: any, result: any) {
    ++depth;
