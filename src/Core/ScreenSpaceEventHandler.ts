...
function getPosition(screenSpaceEventHandler: ScreenSpaceEventHandler, event: any, result: Vector2): Vector2 {
...
function registerListener(screenSpaceEventHandler: ScreenSpaceEventHandler, domType: any, element: HTMLCanvasElement, callback: any) {
...
function canProcessMouseEvent(screenSpaceEventHandler: ScreenSpaceEventHandler) {
    return getTimestamp() - screenSpaceEventHandler._lastSeenTouchEvent > ScreenSpaceEventHandler.mouseEmulationIgnoreMilliseconds;
}

function checkPixelTolerance(startPosition: Vector2, endPosition: Vector2, pixelTolerance: number) {
...
function handleMouseDown(screenSpaceEventHandler: ScreenSpaceEventHandler, event: PointerEvent) {
    ...
    const position: Vector2 = getPosition(screenSpaceEventHandler, event, screenSpaceEventHandler._primaryPosition);
    ...
    const action = screenSpaceEventHandler.getInputAction(screenSpaceEventType, modifier as number);
...
function cancelMouseEvent(screenSpaceEventHandler: ScreenSpaceEventHandler, screenSpaceEventType: number, clickScreenSpaceEventType: number, event: PointerEvent) {
    const modifier = getModifier(event);

    const action = screenSpaceEventHandler.getInputAction(screenSpaceEventType, modifier as number);
    const clickAction = screenSpaceEventHandler.getInputAction(clickScreenSpaceEventType, modifier as number);
...
function handleMouseUp(screenSpaceEventHandler: ScreenSpaceEventHandler, event: PointerEvent) {
    if (!canProcessMouseEvent(screenSpaceEventHandler)) {
        return;
    }
...
function handleMouseMove(screenSpaceEventHandler: ScreenSpaceEventHandler, event: PointerEvent) {
    if (!canProcessMouseEvent(screenSpaceEventHandler)) {
        return;
    }
    ...
    const action = screenSpaceEventHandler.getInputAction(ScreenSpaceEventType.MOUSE_MOVE, modifier as number);
...
function handleDblClick(screenSpaceEventHandler: ScreenSpaceEventHandler, event: PointerEvent) {
    const button = event.button;
...
function handleWheel(screenSpaceEventHandler: ScreenSpaceEventHandler, event: any) {
    ...
    const modifier = getModifier(event);
    const action = screenSpaceEventHandler.getInputAction(ScreenSpaceEventType.WHEEL, modifier as number);
...
function handleTouchStart(screenSpaceEventHandler: ScreenSpaceEventHandler, event: TouchEvent) {
    ...
    for (i = 0; i < length; ++i) {
        ...
        positions.set(identifier, getPosition(screenSpaceEventHandler, touch, new Vector2()));
    }
...
function handleTouchEnd(screenSpaceEventHandler: ScreenSpaceEventHandler, event: TouchEvent) {
    gotTouchEvent(screenSpaceEventHandler);
...
function fireTouchEvents(screenSpaceEventHandler: ScreenSpaceEventHandler, event: any) {
    const modifier = getModifier(event);
    const positions = screenSpaceEventHandler._positions;
    ...
    if (numberOfTouches !== 1 && screenSpaceEventHandler._buttonDown[MouseButton.LEFT]) {
        ...
        action = screenSpaceEventHandler.getInputAction(ScreenSpaceEventType.LEFT_UP, modifier as number);

        if (defined(action)) {
            touchEndEvent.position.copy(screenSpaceEventHandler._primaryPosition);
        ...
        if (numberOfTouches === 0 && !screenSpaceEventHandler._isTouchHolding) {
            ...
            clickAction = screenSpaceEventHandler.getInputAction(ScreenSpaceEventType.LEFT_CLICK, modifier as number);
            ...
            if (defined(clickAction)) {
                ...
                if (checkPixelTolerance(startPosition, endPosition, screenSpaceEventHandler._clickPixelTolerance)) {
                    touchClickEvent.position.copy(screenSpaceEventHandler._primaryPosition);
    ...
    if (numberOfTouches === 0 && pinching) {
        ...
        action = screenSpaceEventHandler.getInputAction(ScreenSpaceEventType.PINCH_END, modifier as number);
    ...
    if (numberOfTouches === 1 && !pinching) {
        ...
        action = screenSpaceEventHandler.getInputAction(ScreenSpaceEventType.LEFT_DOWN, modifier as number);
        ...
        screenSpaceEventHandler._touchHoldTimer = setTimeout(function () {
            if (!screenSpaceEventHandler.isDestroyed()) {
                ...
                clickAction = screenSpaceEventHandler.getInputAction(ScreenSpaceEventType.RIGHT_CLICK, modifier as number);

                if (defined(clickAction)) {
                    const startPosition = screenSpaceEventHandler._primaryStartPosition;
                    const endPosition = screenSpaceEventHandler._previousPositions.values[0];
                    if (checkPixelTolerance(startPosition, endPosition, screenSpaceEventHandler._holdPixelTolerance)) {
                        touchHoldEvent.position.copy(screenSpaceEventHandler._primaryPosition);
    ...
    if (numberOfTouches === 2 && !pinching) {
        ...
        action = screenSpaceEventHandler.getInputAction(ScreenSpaceEventType.PINCH_START, modifier as number);
...
function handleTouchMove(screenSpaceEventHandler: ScreenSpaceEventHandler, event: TouchEvent) {
    gotTouchEvent(screenSpaceEventHandler);
...
function fireTouchMoveEvents(screenSpaceEventHandler: ScreenSpaceEventHandler, event: any) {
    const modifier = getModifier(event);
    const positions = screenSpaceEventHandler._positions;
    const previousPositions = screenSpaceEventHandler._previousPositions;
    const numberOfTouches = positions.length;
    ...
    if (numberOfTouches === 1 && screenSpaceEventHandler._buttonDown[MouseButton.LEFT]) {
        ...
        action = screenSpaceEventHandler.getInputAction(ScreenSpaceEventType.MOUSE_MOVE, modifier as number);
        ...
        action = screenSpaceEventHandler.getInputAction(ScreenSpaceEventType.PINCH_MOVE, modifier as number);
        if (defined(action)) {
            ...
            touchPinchMovementEvent.angleAndHeight.startPosition.set(prevAngle, prevCY);
...
function handlePointerDown(screenSpaceEventHandler: ScreenSpaceEventHandler, event: any) {
    ...
    if (event.pointerType === 'touch') {
        ...
        positions.set(identifier, getPosition(screenSpaceEventHandler, event, new Vector2()));
...
function handlePointerUp(screenSpaceEventHandler: ScreenSpaceEventHandler, event: PointerEvent) {
    if (event.pointerType === 'touch') {
        const positions = screenSpaceEventHandler._positions;
...
function handlePointerMove(screenSpaceEventHandler: ScreenSpaceEventHandler, event: PointerEvent) {
    if (event.pointerType === 'touch') {
        const positions = screenSpaceEventHandler._positions;
