...
export default class TerrainEncoding {
    ...
    // get threeMatrix4(): CesiumMatrix4 {
    ...
    addGeodeticSurfaceNormals(oldBuffer: any, newBuffer: any, ellipsoid: Ellipsoid): void {
        if (this.hasGeodeticSurfaceNormals) {
            return;
        }
        ...
        for (let index = 0; index < vertexCount; index++) {
            ...
            const geodeticSurfaceNormal = ellipsoid.geodeticSurfaceNormal(position, scratchGeodeticSurfaceNormal) as Cartesian3;
    ...
    removeGeodeticSurfaceNormals(oldBuffer: any, newBuffer: any): void {
        if (!this.hasGeodeticSurfaceNormals) {
            return;
        }
    ...
    getExaggeratedPosition(buffer: Float32Array, index: number, result: Cartesian3): Cartesian3 {
        ...
        if (hasExaggeration && this.hasGeodeticSurfaceNormals) {
            ...
            const heightDifference = TerrainExaggeration.getHeight(rawHeight, exaggeration, exaggerationRelativeHeight) - rawHeight;
    ...
    getOctEncodedNormal(buffer: any, index: any, result: any) {
    ...
    decodeGeodeticSurfaceNormal(buffer: any, index: any, result: any) {
