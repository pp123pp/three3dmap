...
function sameMousePosition(movement: IMovement) {
...
function maintainInertia(aggregator: CameraEventAggregator, type: any, modifier: any, decayCoef: any, action: any, object: any, lastMovementName: any) {
...
function reactToInput(controller: ScreenSpaceCameraController, enabled: boolean, eventTypes: any, action: any, inertiaConstant?: any, inertiaStateName?: any) {
...
const scratchZoomPickRay = new CesiumRay();
...
function handleZoom(object: ScreenSpaceCameraController, startPosition: Cartesian2, movement: any, zoomFactor: number, distanceMeasure: number, unitPositionDotDirection?: number) {
    let percentage = 1.0;
    if (defined(unitPositionDotDirection)) {
        percentage = CesiumMath.clamp(Math.abs(unitPositionDotDirection as number), 0.25, 1.0);
    }
    ...
    let rangeWindowRatio = diff / object.scene.canvas.clientHeight;
    ...
    const sameStartPosition = Cartesian2.equals(startPosition, object._zoomMouseStart);
    let zoomingOnVector = object._zoomingOnVector;
    let rotatingZoom = object._rotatingZoom;
    let pickedPosition;

    if (!sameStartPosition) {
        object._zoomMouseStart = Cartesian2.clone(startPosition, object._zoomMouseStart) as Cartesian2;

        if (defined(object._globe)) {
            if (mode === SceneMode.SCENE2D) {
                pickedPosition = (camera.getPickRay(startPosition, scratchZoomPickRay) as CesiumRay).origin;
    ...
    if (!sameStartPosition || rotatingZoom) {
        if (mode === SceneMode.SCENE2D) {
            ...
            if (object._cameraUnderground || object._zoomingUnderground || (camera.positionCartographic.height < 3000.0 && Math.abs(Cartesian3.dot(camera.direction, cameraPositionNormal)) < 0.6)) {
                ...
                const centerPosition = pickGlobe(object, centerPixel, scratchCenterPosition);
    ...
    if ((!sameStartPosition && zoomOnVector) || zoomingOnVector) {
        ...
        const rayDirection = (ray as CesiumRay).direction;
...
const translate2DStart = new CesiumRay();
const translate2DEnd = new CesiumRay();
...
// function singleAxisTwist2D(controller: ScreenSpaceCameraController, startPosition: Cartesian2, movement: IMovement) {
//     let rotateRate = (controller._rotateFactor as number) * (controller._rotateRateRangeAdjustment as number);
...
//     if (!CesiumMatrix4.equals(CesiumMatrix4.IDENTITY, controller._scene.camera.transform)) {
...
const pickGlobeScratchRay = new CesiumRay();
...
function pickGlobe(controller: ScreenSpaceCameraController, mousePosition: Cartesian2, result: Cartesian3) {
    ...
    const depthIntersection = undefined;
    ...
    const pickDistance = defined(depthIntersection) ? Cartesian3.distance(depthIntersection as unknown as Cartesian3, camera.positionWC) : Number.POSITIVE_INFINITY;
    ...
    if (pickDistance < rayDistance) {
        return Cartesian3.clone(depthIntersection as unknown as Cartesian3, result);
    }
...
function getDistanceFromSurface(controller: ScreenSpaceCameraController) {
    ...
    const globeHeight = defaultValue(controller.scene.globeHeight, 0.0);
...
function getZoomDistanceUnderground(controller: ScreenSpaceCameraController, ray: CesiumRay) {
...
function getTiltCenterUnderground(controller: ScreenSpaceCameraController, ray: CesiumRay, pickedPosition: Cartesian3, result: Cartesian3): Cartesian3 {
    ...
    return CesiumRay.getPoint(ray, distance, result);
}

function getStrafeStartPositionUnderground(controller: ScreenSpaceCameraController, ray: CesiumRay, pickedPosition: Cartesian3, result: Cartesian3): Cartesian3 {
    let distance;
    if (!defined(pickedPosition)) {
...
function continueStrafing(controller: ScreenSpaceCameraController, movement: IMovement) {
...
const translateCVStartRay = new CesiumRay();
const translateCVEndRay = new CesiumRay();
...
const translateCVPlane = new CesiumPlane(Cartesian3.UNIT_X, 0.0);
const translateCVStartMouse = new Cartesian2();
const translateCVEndMouse = new Cartesian2();

function translateCV(controller: ScreenSpaceCameraController, startPosition: any, movement: IMovement) {
    if (!Cartesian3.equals(startPosition, controller._translateMousePosition)) {
        controller._looking = false;
    }

    if (!Cartesian3.equals(startPosition, controller._strafeMousePosition as any)) {
    ...
    const startMouse = Cartesian2.clone(movement.startPosition, translateCVStartMouse) as Cartesian2;
    const endMouse = Cartesian2.clone(movement.endPosition, translateCVEndMouse) as Cartesian2;
    let startRay = camera.getPickRay(startMouse, translateCVStartRay) as CesiumRay;
    ...
    const plane = CesiumPlane.fromPointNormal(origin, normal, translateCVPlane);

    startRay = camera.getPickRay(startMouse, translateCVStartRay) as CesiumRay;
    const startPlanePos = IntersectionTests.rayPlane(startRay, plane, translateCVStartPos);

    const endRay = camera.getPickRay(endMouse, translateCVEndRay) as CesiumRay;
    ...
    const diff = Cartesian3.subtract(startPlanePos as Cartesian3, endPlanePos as Cartesian3, translateCVDifference);
    const temp = diff.x;
    diff.x = diff.y;
    diff.y = diff.z;
    diff.z = temp;
...
const rotateCVWindowRay = new CesiumRay();
const rotateCVCenter = new Cartesian3();
const rotateCVVerticalCenter = new Cartesian3();
const rotateCVTransform = new CesiumMatrix4();
const rotateCVVerticalTransform = new CesiumMatrix4();
const rotateCVOrigin = new Cartesian3();
const rotateCVPlane = new CesiumPlane(Cartesian3.UNIT_X, 0.0);
const rotateCVCartesian3 = new Cartesian3();
const rotateCVCart = new Cartographic();
const rotateCVOldTransform = new CesiumMatrix4();
const rotateCVQuaternion = new CesiumQuaternion();
const rotateCVMatrix = new CesiumMatrix3();
const tilt3DCartesian3 = new Cartesian3();

function rotateCV(controller: ScreenSpaceCameraController, startPosition: Cartesian2, movement: IMovement) {
    if (defined(movement.angleAndHeight)) {
...
function rotateCVOnPlane(controller: ScreenSpaceCameraController, startPosition: Cartesian2, movement: IMovement) {
    const scene = controller.scene;
    const camera = scene.camera;
    const canvas = scene.canvas;
    ...
    const ray = camera.getPickRay(windowPosition, rotateCVWindowRay) as CesiumRay;
    const normal = Cartesian3.UNIT_X;
    ...
    if (!defined(scalar) || (scalar as number) <= 0.0) {
        controller._looking = true;
        look3D(controller, startPosition, movement);
    ...
    const center = Cartesian3.multiplyByScalar(direction, scalar as number, rotateCVCenter);
    ...
    const transform = (Transforms as any).eastNorthUpToFixedFrame(center, ellipsoid, rotateCVTransform);
    ...
    const oldTransform = CesiumMatrix4.clone(camera.transform, rotateCVOldTransform);
...
function rotateCVOnTerrain(controller: ScreenSpaceCameraController, startPosition: Cartesian2, movement: IMovement) {
    const scene = controller.scene;
    const camera = scene.camera;
    const cameraUnderground = controller._cameraUnderground;
    ...
    if (Cartesian2.equals(startPosition, controller._tiltCenterMousePosition)) {
        ...
        if (!defined(center)) {
            ray = camera.getPickRay(startPosition, rotateCVWindowRay) as CesiumRay;
            const position = ray.origin;
            const direction = ray.direction;
            ...
            center = Cartesian3.multiplyByScalar(direction, scalar as number, rotateCVCenter);
            Cartesian3.add(position, center, center);
        }
        ...
        if (cameraUnderground) {
            ...
            getTiltCenterUnderground(controller, ray as CesiumRay, center, center);
        }
    ...
    ray = camera.getPickRay(windowPosition, rotateCVWindowRay) as CesiumRay;

    const origin = Cartesian3.clone(Cartesian3.ZERO, rotateCVOrigin);
    origin.x = center.x;

    const plane = CesiumPlane.fromPointNormal(origin, normal, rotateCVPlane);
    const verticalCenter = IntersectionTests.rayPlane(ray, plane, rotateCVVerticalCenter) as Cartesian3;
    ...
    if (defined(verticalCenter)) {
        ...
        verticalTransform = (Transforms as any).eastNorthUpToFixedFrame(verticalCenter, ellipsoid, rotateCVVerticalTransform);
    } else {
    ...
    if (dot < 0.0) {
        ...
        if ((cameraUnderground && movementDelta < 0.0) || (!cameraUnderground && movementDelta > 0.0)) {
            ...
            (constrainedAxis as any) = undefined;
        }

        const oldConstrainedAxis = camera.constrainedAxis;
        camera.constrainedAxis = undefined as any;
    ...
    if (defined(camera.constrainedAxis)) {
        const right = Cartesian3.cross(camera.direction, camera.constrainedAxis as Cartesian3, tilt3DCartesian3);
    ...
    if (!Cartesian3.equals(camera.positionWC, originalPosition)) {
        ...
        const quaternion = CesiumQuaternion.fromAxisAngle(axis, angle, rotateCVQuaternion);
        const rotation = CesiumMatrix3.fromQuaternion(quaternion, rotateCVMatrix);
        CesiumMatrix3.multiplyByVector(rotation, camera.direction, camera.direction);
        CesiumMatrix3.multiplyByVector(rotation, camera.up, camera.up);
        Cartesian3.cross(camera.direction, camera.up, camera.right);
        Cartesian3.cross(camera.right, camera.direction, camera.up);
...
const zoomCVWindowRay = new CesiumRay();
...
function zoomCV(controller: ScreenSpaceCameraController, startPosition: Cartesian2, movement: IMovement) {
    if (defined(movement.distance)) {
        movement = movement.distance;
    }
    ...
    const ray = camera.getPickRay(windowPosition, zoomCVWindowRay) as CesiumRay;
    const position = ray.origin;
    const direction = ray.direction;
    ...
    if (cameraUnderground) {
        ...
        if (defined(distance)) {
            distance = Math.min(distance as number, distanceUnderground);
        } else {
    ...
    handleZoom(controller, startPosition, movement, controller._zoomFactor, distance as number);
}

const pan3DP0 = Cartesian4.clone(Cartesian4.UNIT_W);
const pan3DP1 = Cartesian4.clone(Cartesian4.UNIT_W) as Cartesian4;
...
function pan3D(controller: ScreenSpaceCameraController, startPosition: Cartesian2, movement: IMovement, ellipsoid: Ellipsoid) {
    const scene = controller.scene;
    const camera = scene.camera;

    const startMousePosition = Cartesian2.clone(movement.startPosition, pan3DStartMousePosition) as Cartesian2;
    const endMousePosition = Cartesian2.clone(movement.endPosition, pan3DEndMousePosition) as Cartesian2;

    let p0 = camera.pickEllipsoid(startMousePosition, ellipsoid, pan3DP0 as unknown as Cartesian3) as any;
    let p1 = camera.pickEllipsoid(endMousePosition, ellipsoid, pan3DP1 as unknown as Cartesian3) as any;
...
function spin3D(controller: ScreenSpaceCameraController, startPosition: Cartesian2, movement: IMovement) {
    const scene = controller.scene;
    const camera = scene.camera;
    const cameraUnderground = controller._cameraUnderground;
    ...
    if (defined(globe) && height < controller._minimumPickingTerrainHeight) {
        const mousePos = pickGlobe(controller, movement.startPosition, scratchMousePos);
        if (defined(mousePos)) {
            let strafing = false;
            const ray = camera.getPickRay(movement.startPosition, pickGlobeScratchRay) as CesiumRay;
...
function tilt3D(controller: ScreenSpaceCameraController, startPosition: Cartesian2, movement: IMovement) {
    const scene = controller.scene;
    const camera = scene.camera;
...
function tilt3DOnEllipsoid(controller: ScreenSpaceCameraController, startPosition: Cartesian2, movement: IMovement) {
    const ellipsoid = controller._ellipsoid;
    const scene = controller.scene;
    const camera = scene.camera;
    ...
    windowPosition.x = canvas.clientWidth / 2;
    windowPosition.y = canvas.clientHeight / 2;
    const ray = camera.getPickRay(windowPosition, tilt3DRay) as CesiumRay;

    let center;
    const intersection = IntersectionTests.rayEllipsoid(ray, ellipsoid) as Interval;
    if (defined(intersection)) {
        center = CesiumRay.getPoint(ray, intersection.start, tilt3DCenter);
    } else if (height > controller._minimumTrackBallHeight) {
    ...
    const transform = (Transforms as any).eastNorthUpToFixedFrame(center, ellipsoid, tilt3DTransform);
...
function tilt3DOnTerrain(controller: ScreenSpaceCameraController, startPosition: Cartesian2, movement: IMovement) {
    const ellipsoid = controller._ellipsoid;
    const scene = controller.scene;
    const camera = scene.camera;
    const cameraUnderground = controller._cameraUnderground;
    ...
    if (Cartesian2.equals(startPosition, controller._tiltCenterMousePosition)) {
        ...
        if (!defined(center)) {
            ray = camera.getPickRay(startPosition, tilt3DRay) as CesiumRay;
            intersection = IntersectionTests.rayEllipsoid(ray, ellipsoid) as Interval;
            if (!defined(intersection)) {
                const cartographic = ellipsoid.cartesianToCartographic(camera.position, tilt3DCart);
    ...
    windowPosition.x = canvas.clientWidth / 2;
    windowPosition.y = controller._tiltCenterMousePosition.y;
    ray = camera.getPickRay(windowPosition, tilt3DRay) as CesiumRay;
    ...
    intersection = IntersectionTests.rayEllipsoid(ray, newEllipsoid) as Interval;
    if (!defined(intersection)) {
        return;
    }

    const t = Cartesian3.magnitude(ray.origin) > mag ? intersection.start : intersection.stop;
    const verticalCenter = CesiumRay.getPoint(ray, t, tilt3DVerticalCenter);

    const transform = (Transforms as any).eastNorthUpToFixedFrame(center, ellipsoid, tilt3DTransform);
    const verticalTransform = (Transforms as any).eastNorthUpToFixedFrame(verticalCenter, newEllipsoid, tilt3DVerticalTransform);
    ...
    let constrainedAxis = Cartesian3.UNIT_Z as any;
    ...
    if (!Cartesian3.equals(camera.positionWC, originalPosition)) {
        ...
        const quaternion = CesiumQuaternion.fromAxisAngle(axis, angle, tilt3DQuaternion);
        const rotation = CesiumMatrix3.fromQuaternion(quaternion, tilt3DMatrix);
        CesiumMatrix3.multiplyByVector(rotation, camera.direction, camera.direction);
        CesiumMatrix3.multiplyByVector(rotation, camera.up, camera.up);
        Cartesian3.cross(camera.direction, camera.up, camera.right);
        Cartesian3.cross(camera.right, camera.direction, camera.up);
...
function updateCV(controller: ScreenSpaceCameraController) {
    const scene = controller.scene;
    const camera = scene.camera;

    if (!CesiumMatrix4.equals(CesiumMatrix4.IDENTITY, camera.transform)) {
...
function update3D(controller: ScreenSpaceCameraController) {
...
const scratchStrafeRay = new CesiumRay();
const scratchStrafePlane = new CesiumPlane(Cartesian3.UNIT_X, 0.0);
...
function strafe(controller: ScreenSpaceCameraController, movement: IMovement, strafeStartPosition: Cartesian3) {
    const scene = controller.scene;
    const camera = scene.camera;

    const ray = camera.getPickRay(movement.endPosition, scratchStrafeRay) as CesiumRay;
    ...
    const plane = CesiumPlane.fromPointNormal(strafeStartPosition, direction, scratchStrafePlane);
    const intersection = IntersectionTests.rayPlane(ray, plane, scratchStrafeIntersection) as Cartesian3;
    if (!defined(intersection)) {
        return;
    }
...
function rotate3D(controller: ScreenSpaceCameraController, startPosition: Cartesian2, movement: IMovement, constrainedAxis?: Cartesian3, rotateOnlyVertical = false, rotateOnlyHorizontal = false) {
    const scene = controller.scene;
    const camera = scene.camera;
    const canvas = scene.canvas;
...
// function pan3D(controller: ScreenSpaceCameraController, startPosition, movement, ellipsoid) {
//     const scene = controller.scene;
//     const camera = scene.camera;
...
function zoom3D(controller: ScreenSpaceCameraController, startPosition: Cartesian2, movement: IMovement) {
    if (defined(movement.distance)) {
        movement = movement.distance;
    }
...
const tilt3DRay = new CesiumRay();
const tilt3DCenter = new Cartesian3();
const tilt3DVerticalCenter = new Cartesian3();
const tilt3DTransform = new CesiumMatrix4();
const tilt3DVerticalTransform = new CesiumMatrix4();
const tilt3DOldTransform = new CesiumMatrix4();
const tilt3DQuaternion = new CesiumQuaternion();
const tilt3DMatrix = new CesiumMatrix3();
...
// function tilt3DOnEllipsoid(controller: ScreenSpaceCameraController, startPosition, movement) {
//     const ellipsoid = controller._ellipsoid;
//     const scene = controller.scene;
//     const camera = scene.camera;
//     const minHeight = controller.minimumZoomDistance * 0.25;
...
//     let constrainedAxis: any = Cartesian3.UNIT_Z;
...
const look3DStartRay = new CesiumRay();
const look3DEndRay = new CesiumRay();
const look3DNegativeRot = new Cartesian3();
const look3DTan = new Cartesian3();

function look3D(controller: ScreenSpaceCameraController, startPosition: Cartesian2, movement: IMovement, rotationAxis?: Cartesian3) {
    const scene = controller.scene;
    const camera = scene.camera;
    ...
    let startRay = camera.getPickRay(startPos, look3DStartRay) as CesiumRay;
    let endRay = camera.getPickRay(endPos, look3DEndRay) as CesiumRay;
    let angle = 0.0;
    let start;
    let end;
    ...
    let dot = Cartesian3.dot(start as Cartesian3, end as Cartesian3);
    if (dot < 1.0) {
        // dot is in [0, 1]
        angle = Math.acos(dot);
    }
    ...
    const horizontalRotationAxis = controller._horizontalRotationAxis as Cartesian3;
    if (defined(rotationAxis)) {
        camera.look(rotationAxis as Cartesian3, -angle);
    ...
    startRay = camera.getPickRay(startPos, look3DStartRay) as CesiumRay;
    endRay = camera.getPickRay(endPos, look3DEndRay) as CesiumRay;
    angle = 0.0;
...
function adjustHeightForTerrain(controller: ScreenSpaceCameraController) {
    ...
    let mag;
    if (!CesiumMatrix4.equals(camera.transform, CesiumMatrix4.IDENTITY)) {
        transform = CesiumMatrix4.clone(camera.transform, scratchAdjustHeightTransform);
    ...
    if (cartographic.height < controller._minimumCollisionTerrainHeight) {
        const globeHeight = controller.scene.globeHeight;
    ...
    if (defined(transform)) {
        ...
        if (heightUpdated) {
            ...
            Cartesian3.multiplyByScalar(camera.position, Math.max(mag as number, controller.minimumZoomDistance), camera.position);
...
export default class ScreenSpaceCameraController {
    ...
    _minimumCollisionTerrainHeight = 15000.0;
    /**
    ...
    _strafeMousePosition = new Cartesian2();
    ...
    _rotateRateRangeAdjustment?: number = undefined;
    ...
    update(): void {
        ...
        const exaggeration = defined(this._globe) ? this._globe.terrainExaggeration : 1.0;
        const exaggerationRelativeHeight = defined(this._globe) ? this._globe.terrainExaggerationRelativeHeight : 0.0;
        this._minimumCollisionTerrainHeight = TerrainExaggeration.getHeight(this.minimumCollisionTerrainHeight, exaggeration, exaggerationRelativeHeight);
        this._minimumPickingTerrainHeight = TerrainExaggeration.getHeight(this.minimumPickingTerrainHeight, exaggeration, exaggerationRelativeHeight);
        this._minimumTrackBallHeight = TerrainExaggeration.getHeight(this.minimumTrackBallHeight, exaggeration, exaggerationRelativeHeight);
    ...
    destroy(): void {
        ...
        this._aggregator = this._aggregator && (this._aggregator.destroy() as any);
