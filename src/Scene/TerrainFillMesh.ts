...
import TileEdge from '@/Core/TileEdge';
...
export default class TerrainFillMesh {
    tile: any;
    frameLastUpdated: any = undefined;
    westMeshes: any[] = []; // north to south (CCW)
    westTiles: any[] = [];
    southMeshes: any[] = []; // west to east (CCW)
    southTiles: any[] = [];
    eastMeshes: any[] = []; // south to north (CCW)
    eastTiles: any[] = [];
    northMeshes: any[] = []; // east to west (CCW)
    northTiles: any[] = [];
    southwestMesh: any = undefined;
    southwestTile: any = undefined;
    southeastMesh: any = undefined;
    southeastTile: any = undefined;
    northwestMesh: any = undefined;
    northwestTile: any = undefined;
    northeastMesh: any = undefined;
    northeastTile: any = undefined;
    changedThisFrame: any = true;
    visitedFrame: any = undefined;
    enqueuedFrame: any = undefined;
    mesh: any = undefined;
    ...
    update(tileProvider: any, frameState: any, vertexArraysToDestroy: any): any {
    ...
    destroy(vertexArraysToDestroy: any): any {
    ...
    _destroyVertexArray(vertexArraysToDestroy: any): void {
    ...
    static updateFillTiles(tileProvider: GlobeSurfaceTileProvider, renderedTiles: QuadtreeTile[], frameState: FrameState, vertexArraysToDestroy: any[]): any {
        ...
        for (let i = 0; i < renderedTiles.length; ++i) {
            ...
            if (defined((renderedTile.data as GlobeSurfaceTile).vertexArray)) {
...
function visitRenderedTiles(tileProvider: GlobeSurfaceTileProvider, frameState: FrameState, sourceTile: QuadtreeTile, startTile: QuadtreeTile, currentFrameNumber: number, tileEdge: TileEdge, downOnly: boolean, traversalQueue: Queue, vertexArraysToDestroy: any[]) {
    if (startTile === undefined) {
        // There are no tiles North or South of the poles.
        return;
    }

    let tile: any = startTile;
...
function visitTile(tileProvider: GlobeSurfaceTileProvider, frameState: FrameState, sourceTile: QuadtreeTile, destinationTile: QuadtreeTile, tileEdge: TileEdge, frameNumber: number, traversalQueue: Queue, vertexArraysToDestroy: any[]) {
    const destinationSurfaceTile = destinationTile.data as GlobeSurfaceTile;
...
function propagateEdge(tileProvider: GlobeSurfaceTileProvider, frameState: FrameState, sourceTile: QuadtreeTile, destinationTile: QuadtreeTile, tileEdge: TileEdge, vertexArraysToDestroy: any[]) {
    const destinationFill = (destinationTile.data as GlobeSurfaceTile).fill;

    let sourceMesh;
    const sourceFill = (sourceTile.data as GlobeSurfaceTile).fill;
    if (defined(sourceFill)) {
        ...
        sourceMesh = (sourceTile.data as GlobeSurfaceTile).fill.mesh;
    } else {
        sourceMesh = (sourceTile.data as GlobeSurfaceTile).mesh;
    }
...
class HeightAndNormal {
    height = 0.0;
    encodedNormal = new Cartesian2();
}

function fillMissingCorner(fill: TerrainFillMesh, ellipsoid: Ellipsoid, u: number, v: number, corner: HeightAndNormal, adjacentCorner1: any, adjacentCorner2: any, oppositeCorner: any, vertex: any) {
    if (defined(corner)) {
        return corner;
    }
...
function createFillMesh(tileProvider: GlobeSurfaceTileProvider, frameState: FrameState, tile: QuadtreeTile, vertexArraysToDestroy: any[]) {
    ...
    if (rectangle.width > maxTileWidth && maximumHeight - minimumHeight <= geometricError) {
        ...
        createMeshSyncOptions.tilingScheme = tile.tilingScheme as any;
        ...
        // fill.mesh = terrainData.createMesh(createMeshSyncOptions);
    } else {
        ...
        const encoding = new TerrainEncoding(center, undefined as any, undefined as any, undefined as any, undefined as any, true, true, hasGeodeticSurfaceNormals, exaggeration, exaggerationRelativeHeight);
        ...
        const heightRange = heightRangeScratch as any;
...
function transformTextureCoordinates(sourceTile: any, targetTile: any, coordinates: any, result: any) {
    ...
    if (Math.abs(u) < CesiumMath.EPSILON5) {
        u = 0.0;
    } else if (Math.abs(u - 1.0) < CesiumMath.EPSILON5) {
        u = 1.0;
    }

    if (Math.abs(v) < CesiumMath.EPSILON5) {
        v = 0.0;
    } else if (Math.abs(v - 1.0) < CesiumMath.EPSILON5) {
        v = 1.0;
    }
...
function addEdge(terrainFillMesh: TerrainFillMesh, ellipsoid: Ellipsoid, encoding: TerrainEncoding, typedArray: Float32Array, nextIndex: number, edgeTiles: QuadtreeTile[], edgeMeshes: TerrainMesh[], tileEdge: TileEdge, heightRange: HeightAndNormal) {
...
function addEdgeMesh(terrainFillMesh: TerrainFillMesh, ellipsoid: Ellipsoid, encoding: TerrainEncoding, typedArray: Float32Array, nextIndex: number, edgeTile: QuadtreeTile, edgeMesh: TerrainMesh, tileEdge: TileEdge, heightRange: any) {
    ...
    if (tileEdge === TileEdge.EAST && terrainFillMesh.tile.x === 0) {
        sourceRectangle = Rectangle.clone(edgeTile.rectangle, sourceRectangleScratch) as Rectangle;
        sourceRectangle.west -= CesiumMath.TWO_PI;
        sourceRectangle.east -= CesiumMath.TWO_PI;
    ...
    for (let i = 0; i < indices.length; ++i) {
        ...
        if (sourceEncoding.hasWebMercatorT) {
            ...
            webMercatorT = ((WebMercatorProjection.geodeticLatitudeToMercatorAngle(latitude) - southMercatorY) * oneOverMercatorHeight) as number;
        }
...
function getNearestHeightOnEdge(meshes: any[], tiles: any[], isNext: boolean, edge: TileEdge, u: number, v: number) {
    let meshStart;
    let meshEnd;
    let meshStep;
...
function meshIsUsable(tile: QuadtreeTile, mesh: TerrainMesh) {
    return defined(mesh) && (!defined((tile.data as GlobeSurfaceTile).fill) || !(tile.data as GlobeSurfaceTile).fill.changedThisFrame);
}

function getCornerFromEdge(terrainFillMesh: TerrainFillMesh, ellipsoid: Ellipsoid, edgeMeshes: TerrainMesh[], edgeTiles: QuadtreeTile[], isNext: boolean, u: number, v: number, vertex: HeightAndNormal) {
    let edgeVertices;
    let compareU: any;
    let increasing: any;
    ...
    if (meshIsUsable(sourceTile, sourceMesh)) {
        ...
        if (edgeVertices.length > 0) {
            ...
            vertexIndexIndex = binarySearch(edgeVertices, compareU ? u : v, function (vertexIndex: number, textureCoordinate: any) {
                sourceMesh.encoding.decodeTextureCoordinates(sourceMesh.vertices, vertexIndex, uvScratch);
                const targetUv = transformTextureCoordinates(sourceTile, terrainFillMesh.tile, uvScratch, uvScratch);
                if (increasing) {
                    if (compareU) {
...
function computeOccludeePoint(tileProvider: GlobeSurfaceTileProvider, center: Cartesian3, rectangle: Rectangle, minimumHeight: number, maximumHeight: number, result?: Cartesian3) {
