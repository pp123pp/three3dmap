...
function sameMousePosition(movement: { startPosition: Cartesian2; endPosition: Cartesian2 }) {
        ...
        if (mode === SceneMode.SCENE2D) {
            ...
            if (!Cartesian3.equals(worldPosition, endPosition) && camera.positionCartographic.height < object._maxCoord.x * 2.0) {
                ...
                if ((camera.position.x < 0.0 && savedX > 0.0) || (camera.position.x > 0.0 && savedX < 0.0)) {
                    pickedPosition = (camera as any).getPickRay(startPosition, scratchZoomPickRay).origin;
...
function pickGlobe(controller: ScreenSpaceCameraController, mousePosition: Cartesian2, result: Cartesian3) {
    ...
    const rayIntersection = globe.pickWorldCoordinates(ray as CesiumRay, scene, cullBackFaces, scratchRayIntersection) as Cartesian3;

    const pickDistance = defined(depthIntersection) ? Cartesian3.distance(depthIntersection as any, camera.positionWC) : Number.POSITIVE_INFINITY;
    ...
    if (pickDistance < rayDistance) {
        return Cartesian3.clone(depthIntersection as any, result);
    }
...
function getZoomDistanceUnderground(controller: any, ray: any, height?: number) {
...
function strafe(controller: any, movement: any, strafeStartPosition: any) {
    ...
    direction = Cartesian3.subtract(strafeStartPosition, intersection as Cartesian3, direction);
    if (scene.mode === SceneMode.COLUMBUS_VIEW) {
...
function rotate3D(controller: ScreenSpaceCameraController, startPosition: any, movement: any, constrainedAxis?: any, rotateOnlyVertical?: any, rotateOnlyHorizontal?: any) {
    ...
    let rotateRate = (controller._rotateFactor as number) * (rho - controller._rotateRateRangeAdjustment);
...
function zoom3D(controller: ScreenSpaceCameraController, startPosition: any, movement: any) {
    if (defined(movement.distance)) {
        movement = movement.distance;
    }
...
function tilt3DOnTerrain(controller: any, startPosition: any, movement: any) {
    ...
    if (Cartesian2.equals(startPosition, controller._tiltCenterMousePosition)) {
        ...
        if (!defined(center)) {
            ...
            center = CesiumRay.getPoint(ray, (intersection as Interval).start, tilt3DCenter);
        }
    ...
    const t = Cartesian3.magnitude(ray.origin) > mag ? (intersection as Interval).start : (intersection as Interval).stop;
...
function look3D(controller: ScreenSpaceCameraController, startPosition: any, movement: any, rotationAxis?: any) {
    const scene = controller._scene;
    const camera = scene.camera;
...
class ScreenSpaceCameraController {
    ...
    _horizontalRotationAxis: Cartesian3 | undefined;
