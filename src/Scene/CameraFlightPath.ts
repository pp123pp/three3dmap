...
const CameraFlightPath: {
    createTween: (scene: MapScene, options: any) => any;
} = {
    createTween: undefined as any,
};
...
function getAltitude(frustum: any, dx: any, dy: any) {
    ...
    let top;
    let right;
    if (frustum instanceof PerspectiveFrustumCamera) {
...
function useLongestFlight(startCart: Cartographic, destCart: Cartographic) {
...
function useShortestFlight(startCart: Cartographic, destCart: Cartographic) {
...
function createUpdate3D(scene: any, duration: any, destination: any, heading: any, pitch: any, roll: any, optionAltitude: any, optionFlyOverLongitude: any, optionFlyOverLongitudeWeight: any, optionPitchAdjustHeight: any) {
    ...
    const startCart = Cartographic.clone(camera.positionCartographic, scratchStartCart) as Cartographic;
    const startPitch = camera.pitch;
    const startHeading = adjustAngleForLERP(camera.heading, heading);
    const startRoll = adjustAngleForLERP(camera.roll, roll);
...
function wrapCallback(controller: ScreenSpaceCameraController, cb: any) {
    function wrapped() {
...
CameraFlightPath.createTween = function (scene, options) {
    ...
    empty = empty && Cartesian2.equalsEpsilon(camera.position as any, destination, CesiumMath.EPSILON6);
    ...
    if (!defined(easingFunction)) {
        ...
        const endHeight = mode === SceneMode.SCENE3D ? (ellipsoid.cartesianToCartographic(destination) as Cartographic).height : destination.z;
