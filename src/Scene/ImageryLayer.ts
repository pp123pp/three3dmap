...
class ImageryLayer {
    ...
    constructor(
         ...
         * @default 1.0
         */
        this.nightAlpha = defaultValue(options.nightAlpha, defaultValue(imageryProvider.defaultNightAlpha, 1.0));
         ...
         * @default 1.0
         */
        this.dayAlpha = defaultValue(options.dayAlpha, defaultValue(imageryProvider.defaultDayAlpha, 1.0));
        ...
        this._skeletonPlaceholder = new TileImagery(Imagery.createPlaceholder(this), undefined as any, undefined as any);
         ...
         * @type {Number}
         */
        this.colorToAlphaThreshold = defaultValue(options.colorToAlphaThreshold, ImageryLayer.DEFAULT_APPLY_COLOR_TO_ALPHA_THRESHOLD) as number;
    }
    ...
    _reprojectTexture(frameState: FrameState, imagery: Imagery, needGeographicProjection = true): void {
        ...
        if (needGeographicProjection && !(this._imageryProvider.tilingScheme.projection instanceof GeographicProjection) && rectangle.width / texture.image.width > 1e-5) {
            const that = this;
            imagery.addReference();
